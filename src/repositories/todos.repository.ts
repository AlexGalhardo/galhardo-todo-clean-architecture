...
import type { ToDoCreateDTO } from "src/use-cases/todo/todo-create.usecase";
import type { ToDoUpdateDTO } from "src/use-cases/todo/todo-update-by-id.usecase";
...
export default class ToDosRepository implements ToDosRepositoryPort {
    async getAll(): Promise<ToDo[]> {
        if (process.env.USE_JSON_DATABASE === "true") {
            try {
                return await Bun.file("./src/repositories/jsons/todos.json").json();
            } catch (error: any) {
    ...
    async getById(id: string): Promise<ToDo | null> {
        if (process.env.USE_JSON_DATABASE === "true") {
            try {
                const file = await Bun.file("./src/repositories/jsons/todos.json").json();
    ...
    async create({ title, description, done }: ToDoCreateDTO): Promise<ToDo> {
        if (process.env.USE_JSON_DATABASE === "true") {
            try {
                ...
                await Bun.write("./src/repositories/jsons/todos.json", JSON.stringify(file, null, 4));
